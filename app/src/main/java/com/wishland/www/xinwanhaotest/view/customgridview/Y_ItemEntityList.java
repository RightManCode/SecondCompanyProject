...
public class Y_ItemEntityList {
    ...
    public Y_ItemEntityList addItems(int position, int itemLayoutId, List<GBean.DataBean.GameBean.ItemsBean> itemDatas) {
    ...
    public Y_ItemEntityList addItems(int itemLayoutId, List<GBean.DataBean.GameBean.ItemsBean> itemDatas) {
        if(mItems.size() != 0) {
            mItems.clear();
        }
        for (int i = 0; i < itemDatas.size(); i++) {
            mItemLayoutIds.add(itemLayoutId);
            mItems.add(itemDatas.get(i).getName());
        }
        return this;
    }
